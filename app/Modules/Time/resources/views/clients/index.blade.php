...
<div class="row">
    <div class="col-sm-12">
        <a href="{{route('time.clients.create')}}" class="btn btn-primary pull-right">{{trans('app.time.clients.add_new')}}</a>
    </div>
</div>
<div class="row">
    <div class="col-sm-12">
        <div class="custom-panel">
            <div class="custom-panel-heading">{{trans('app.time.clients.main')}}</div>
            <table class="table table-bordered table-hover" id="clientsTable">
                <thead>
                    <th>{{trans('app.id')}}</th>
                    <th>{{trans('app.time.clients.name')}}</th>
                    <th></th>
                </thead>
                <tfoot>
                    <th>
                    ...
                    <th>
                        <input type="text" placeholder="{{trans('app.time.clients.name')}}"/>
                    </th>
                    <th></th>
                </tfoot>
            </table>
...
<script>
    $(document).ready(function(){
        var table = $('#clientsTable').DataTable({
            processing: true,
            serverSide: true,
            ajax: '{{ route("time.clients.datatable")}}',
            columns: [
                {data: 0, name: 'id'},
                {data: 1, name: 'name'},
